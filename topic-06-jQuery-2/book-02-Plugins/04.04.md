...
To display the title attribute values as error messages on the web page, we assign the data-h5-errorid attribute in the input field and we add a corresponding error div. Error divs should be hidden in the HTML with display:none; -- h5Validate shows them when the corresponding field is marked invalid.The data-h5-errorid attribute is assigned a value and that value is the id of the error div. In h5Validate, you tell the client side code where to find the error div with the data-h5-errorid attribute. When the input field is marked invalid by the browser, the value of the title attribute is displayed in the error div. We attach a class also to style the error message.

```html
<div class ="form-group">
     <label for="name" class="col-sm-3 control-label">Name</label>
     <div class="col-sm-9">
        <input type="text" class="form-control" name="name"  id="name" data-h5-errorid="invalid-name" required pattern = "[A-Z][a-z]+( [A-Z][a-z]+)?" autofocus
